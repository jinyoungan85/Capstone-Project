...
class RegistrationForm(FlaskForm):
    ...
    def validate_email(self, email):
        ...
        school_email = ['mga.edu']
        user_email_domain = request.form['email'].split('@')[-1]

        if user:
            raise ValidationError('An account has already been created with this Email.')
        elif user_email_domain not in school_email:
            # Raise error if user's email address is not @mga.edu school domain.
            raise ValidationError('Your email address must be MGA school email. Please try again.')            
...
class ApproveForm(FlaskForm):
    ...
    reason = StringField('Reason for Approval/Denial', validators = [DataRequired(), Length(max=100)])
